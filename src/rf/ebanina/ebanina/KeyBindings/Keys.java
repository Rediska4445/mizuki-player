 ...
 * <p>
 * {@code Keys} содержит два списка комбинаций:
 * <ul>
 ...
 *   <li>Динамическая проверка комбинаций через {@link KeyBindingController#pressed_keys}.</li>
...
//            ).setOnReleased(() -> MediaProcessor.mediaProcessor.pause_play()),
    ...
    public String getHotKeysStringCodes(String code) {
        return Arrays.stream(Keys.instance.findAndGetHotKey(code).registers).mapToObj(e -> String.valueOf(NativeKeyEvent.getKeyText(e))).collect(Collectors.joining(" + "));
    }
     ...
     * <p>
     * Вызывается из {@link KeyBindingController#nativeKeyPressed(NativeKeyEvent)}.
    ...
    public void globalKeys(NativeKeyEvent code) {
        updateKeysOnPressedEvent(Keys.instance.getHotKeys());
    }
     ...
     * </ul>
     * Вызывается из {@link KeyBindingController#setupKeyListeners(Scene)}}.
     * </p>
     */
    public void sceneKeys() {
        ...
        updateKeysOnPressedEvent(Keys.instance.getSceneHotKeys());
    }
     ...
     *   <li>Проверка всех клавиш {@link HotKey#registers} в {@link KeyBindingController#pressed_keys}.</li>
        ...
        for(int pointerToKey = 0; pointerToKey < hotKeys.size(); pointerToKey++) {
            ...
            for(int keyPointer = 0; keyPointer < keys.length; keyPointer++) {
                ...
                if(hotKey.getPredicate() == null) {
                    ...
                    if (!pressed_keys.contains(keys[keyPointer])) {
