 ...
 *   <li>Обработка <b>глобальных горячих клавиш</b> через {@link KeyBindingController#globalKeys(NativeKeyEvent)}.</li>
 ...
 * GlobalScreen.addNativeKeyListener(new KeyBindingProcessor());
 * GlobalScreen.addNativeMouseListener(new KeyBindingProcessor());
 * GlobalScreen.addNativeMouseWheelListener(new KeyBindingProcessor());
 *
 * // Проверка нажатия клавиши
 * if (KeyBindingProcessor.isKeyPressed(NativeKeyEvent.VC_CONTROL)) { ... }
 *
 * // JavaFX-сцена
 * KeyBindingProcessor.setupKeyListeners(scene);
 * }</pre>
 *
 * <h2>Особенности</h2>
 * <ul>
        ...
        implements NativeMouseInputListener, NativeMouseWheelListener, NativeKeyListener
{
    ...
    private final AudioVolumer audioVolumer;

    private final PlayProcessor<?, ?> playProcessor;

    private final KeyBindingController keyBindingController;

    public KeyBindingProcessor(MediaProcessor mediaProcessor, AudioVolumer audioVolumer, PlayProcessor<?, ?> playProcessor, KeyBindingController keyBindingController) {
        this.mediaProcessor = mediaProcessor;
        this.audioVolumer = audioVolumer;
        this.playProcessor = playProcessor;
        this.keyBindingController = keyBindingController;
    }

    private static KeyBindingProcessor instance;

    public static KeyBindingProcessor defaultInstance() {
        return new KeyBindingProcessor(MediaProcessor.mediaProcessor, AudioVolumer.instance, PlayProcessor.playProcessor, KeyBindingController.instance);
    }

    public static KeyBindingProcessor getInstance() {
        if(instance == null)
    ...
    public void nativeMouseWheelMoved(NativeMouseWheelEvent e) {
        if(isKeyPressed(NativeKeyEvent.VC_ALT) && isKeyPressed(NativeKeyEvent.VC_SHIFT)) {
            mediaProcessor.setTempo((float) (mediaProcessor.getMediaPlayer().getTempo() + (-e.getWheelRotation()) * 0.05));
        }

        if(isKeyPressed(NativeKeyEvent.VC_ALT) && isKeyPressed(NativeKeyEvent.VC_CONTROL)) {
            audioVolumer.setSystemVolume((float) (audioVolumer.getSystemVolume() + (-e.getWheelRotation()) * 0.05));
        }
    }
    ...
    public void nativeMouseReleased(NativeMouseEvent e) {
        if((e.getButton() == 4) && isKeyPressed(NativeKeyEvent.VC_ALT)) {
            Root.PlaylistHandler.playlistHandler.openTrack(playProcessor.getTrackHistoryGlobal().back());
        }

        if((e.getButton() == 5) && isKeyPressed(NativeKeyEvent.VC_ALT)) {
            Root.PlaylistHandler.playlistHandler.openTrack(playProcessor.getTrackHistoryGlobal().forward());
        }
    }
     ...
     * Выполняет:
     * <ul>
     *   <li>Обновление состояния {@link KeyBindingController} ({@link KeyBindingController#updateKeysOnReleasedEvent}).</li>
    ...
    public void nativeKeyReleased(NativeKeyEvent e) {
        keyBindingController.updateKeysOnReleasedEvent(keyBindingController.getHotKeys());
     ...
     *   <li>Проверка глобальных горячих клавиш {@link KeyBindingController#globalKeys(NativeKeyEvent)}.</li>
     * </ol>
     * </p>
     *
     * @param e событие клавиши
    ...
    public void nativeKeyPressed(NativeKeyEvent e) {
        ...
        keyBindingController.globalKeys(e);
    }
     ...
     * <ul>
     *   <li>KEY_PRESSED — добавление в {@link #pressed_keys} + {@link KeyBindingController#sceneKeys()}.</li>
    ...
    public static void setupKeyListeners(Scene scene) {
        KeyBindingProcessor c = defaultInstance();
        ...
        Music.mainLogger.info("KeyBindingProcessor initialized and listeners registered");

        // Загрузка собственных горячих клавиш
        KeyBindingController.instance.loadBindings();

        // Загрузка дефолтных горячих клавиш
        KeyBindingController.instance.collectKeyBindsFromObjects(
        ...
        scene.addEventHandler(KeyEvent.KEY_PRESSED, event -> {
            ...
            KeyBindingController.instance.sceneKeys();
        });
